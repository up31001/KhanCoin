...
unsigned int nTargetSpacing = 6 * 60;
unsigned int nStakeMinAge = 60 * 60 * 24 * 7;  // 7 days
unsigned int nStakeMaxAge = 60 * 60 * 24 * 35; // 35 days         
...
const string strMessageMagic = "KhanCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
	...
	// Years of mining rewards (1% total coin)
	if (nHeight < 75)
		nSubsidy = 30 * COIN;
...
const int DAILY_BLOCKCOUNT =  1440 / 6;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "KhanCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "KhanCoin Genesis Block";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime = 1439842000;
        }
        else
        {
        ...
        block.nVersion = 1;
        block.nTime    = 1439842050;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1261720;
        if (fTestNet)
        {
            block.nTime    = 0;
            block.nNonce   = 0;
        ...
        // debug print
         assert(block.hashMerkleRoot == uint256("0x1ef0bec2753987c80617d9f856bbf77edb30edf359d49074cfcc1aceac4aefac"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xe3, 0xa2, 0xda };
